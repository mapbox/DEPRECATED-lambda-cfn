      ...
      if (arg.Variables) {
        ...
        Object.keys(arg.Variables).forEach((key) => {
          if (names.Variables.has(key)) {
            try { assert.deepEqual(template.Variables[key], arg.Variables[key]); }
...
module.exports.buildAlarmSnsTopic = buildAlarmSnsTopic;
function buildAlarmSnsTopic(options) {
  // TODO: permissions for this should really be in here, not in the buildRole function
  return {
    Parameters: {
      ServiceAlarmEmail:{
        Type: 'String',
    ...
    Variables: {
      ServiceAlarmSNSTopic: cf.ref('LambdaCfnAlarmSNSTopic')
    }
  };
}
...
function buildSnsDestination(options) {
  ...
  if (options.destinations.sns) {
    ...
    sns.Resources.LambdaCfnRole.Properties.Policies.push({
        ...
        Statement: [
          {
            Effect: 'Allow',
            Action: 'sns:Publish',
            Resource: cf.ref(options.name + 'SNSDestination')
          }
        ]
      }
    });
  ...
  sns.Variables.ApplicationAlarmSNSTopic = cf.ref(options.name + 'SNSTopic');
