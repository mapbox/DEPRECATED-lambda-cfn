...
function compileFunction() {
  ...
  if (arguments) {
    for (var arg of arguments) {
      ...
      // TODO: Make a global variables with HasDispatchSnsArn
      if (arg.Conditions && !('HasDispatchSnsArn' in arg.Conditions)) {
        arg.Conditions.HasDispatchSnsArn = {
          'Fn::Not': [
            {
              'Fn::Equals': [
                '',
        ...
        arg.Conditions = dispatchCondition;
      }
      ...
      if (arg.Conditions) {
        ...
        Object.keys(arg.Conditions).forEach((key) => {
          if (names.Conditions.has(key) && key !== 'HasDispatchSnsArn') {
          ...
          if (!('HasDispatchSnsArn' in names.Conditions)) {
...
function buildCloudwatchEvent(options, functionType) {
  ...
  var eventName = options.name + utils.capitalizeFirst(functionType);
  var event = {
    Resources: {}
  };
...
function buildServiceAlarms(options) {
  ...
  alarms.Parameters = {
    ServiceAlarmEmail:{
      Type: 'String',
      Description: 'Service alarm notifications will send to this email address [Leave it empty if you fill DispatchSnsArn]'
    }
  };
...
function buildSnsDestination(options) {
  ...
  if (options.destinations && options.destinations.sns) {
    for (var destination in options.destinations.sns) {
